            ...
            {orders.map((row) => {
              ...
              return (
                <TableRow key={row.id} sx={{ '&:hover': { backgroundColor: '#FAFAFA' } }}>
                  ...
                  <TableCell sx={{ fontWeight: 700 }}>Γé╣{row.totalAmount?.toLocaleString() || 'ΓÇö'}</TableCell>
                  <TableCell>
          ...
          <Box sx={{ borderTop: '1px solid #E0E0E0', pt: 3, mb: 3 }}>
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
              ...
              <Box>
                ...
                <Typography sx={{ fontWeight: 900, fontSize: '1.5rem' }}>Γé╣{selectedOrder?.totalAmount?.toLocaleString()}</Typography>
              </Box>
            </Box>
          </Box>
      ...
      <Dialog open={!!editingId} onClose={() => !editLoading && setEditingId(null)} maxWidth="xs" fullWidth>
        <Box sx={{ p: 3 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            ...
            <TextField label="Total Amount (Γé╣)" type="number" fullWidth value={editTotal} disabled
